...
export default class UserVirtualizedList extends Component {
  
  constructor(props) {
    super(props)
    this.list = generateUsers(1e3)
    this.state = {
  ...
  render() {
    ...
    return <div className='flex flex-dir-c'>
      <div> <a href="https://blog.csdn.net/sinat_17775997/article/details/100900677?utm_medium=distribute.pc_relevant.none-task-blog-2~default~baidujs_baidulandingword~default-0-100900677-blog-125784599.235^v38^pc_relevant_sort_base3&spm=1001.2101.3001.4242.1&utm_relevant_index=3">浅说虚拟列表的实现原理</a>
        ...
        <div style={{ paddingTop: `${startOffset}px`, paddingBottom: `${endOffset}px`}}>
          {
            renderList.length > 0
              ? renderList.map((item, index) => {
                ...
                return <Item key={startIndex + index} index={startIndex + index} cachePosition={(el, i) => this.cachePosition(el, i)} ><div style={{ color: 'red' }}> {item.index} : {item.name} : {item.index * height} </div></Item>
              })
              : null
          }
        </div>
